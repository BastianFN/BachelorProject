...
for json_file in "${json_files[@]}"; do
    ...
    for workers in "${worker_counts[@]}"; do
        ...
        for step_size in "${step_sizes[@]}"; do
            ...
            timelymon_times_file="json_${json_size}_${workers}w_s${step_size}.txt"
            ...
            for i in $(seq 1 $N); do
                ...
                output=$( { time (cat "data/$json_file" | target/release/timelymon "<<.user & .type = \"edit\" & .bot = false>> AND ONCE[1,1] <<.user & .type = \"edit\" & .bot = false>>" -w $workers -m 1 -s $step_size -f json) 2>&1 > /dev/null; } 2>&1 )
