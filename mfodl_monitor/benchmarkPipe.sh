...
for json_file in "${json_files[@]}"; do
    ...
    for workers in "${worker_counts[@]}"; do
        ...
        for step_size in "${step_sizes[@]}"; do
            ...
            timelymon_times_file="pipe_${json_size}_${workers}w_s${step_size}.txt"
            ...
            for i in $(seq 1 $N); do
                ...
                output=$( { time (cat "data/$json_file" | jq -cM '{ts: .timestamp, user: .user, bot: .bot, type: .type}' | python3 proc.py log 1 && cat log | target/release/timelymon "edit(user,0) AND ONCE[1,1] edit(user,0)" -w $workers -m 1 -s $step_size) 2>&1 > /dev/null; } 2>&1)
